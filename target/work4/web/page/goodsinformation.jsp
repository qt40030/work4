...
<body>
	<div class="layui-collapse">
		<div class="layui-colla-item">
			<h2 class="layui-colla-title">商品信息-维护</h2>
	...
	function update(code) {
		layer.open({
			...
			maxmin:true,
 			...
 			content:'/work4/web/page/gi_update.jsp?code='+code
		});
	}
	//添加图片
	function image(code) {
		layer.open({
			...
			maxmin:true,
 			...
 			content:'/work4/web/page/img_goods.jsp?code='+code
		});
	}
	//图片维护img_maintenance
	function maintenance(code) {
		layer.open({
			type:2,
			shadeClose:true,
			area:['800px','500px'],
			maxmin:true,
 			...
 			content:'/work4/web/page/img_maintenance.jsp?code='+code
		});
	}
	function del(code){
		if(confirm("确定删除？")){
	       $.ajax({
	    	   url:"/work4/informationController/delete.do?code="+code,
	    		dataType:'text',
	    		type:'post',
	...
	formSubmit('/work4/informationController/selectpage.do','submit(select)','json',function(data){
		...
		setPageInfo('pageInfoUser',data.count,curr,limit,function(obj,first){
		      ...
		      $("#user_tbody").empty();
			    $.each(data.goods,function(i,dom){
						...
						html+='<td id='+(i+1+(curr-1)*limit)+'>'+dom.price+'</td>';
						html+='<td id='+(i+1+(curr-1)*limit)+'>'+dom.descr+'</td>';
						html+='<td id='+(i+1+(curr-1)*limit)+'>'+dom.inventory+'</td>';
						...
						a1+='<input type="button" class="layui-btn" value="添加图片" onclick="image(\''+dom.code+'\')" /> ';
						a1+='<input type="button" class="layui-btn" value="维护图片" onclick="maintenance(\''+dom.code+'\')" /> ';
	...
	function inst() {
		layer.open({
			...
			content:'/work4/web/page/gi_inster.jsp'
		});
	}
</script>
</body>
